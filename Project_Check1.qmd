...
Description: I wanted to see whether plastic waste per volunteer was related to average PM2.5 concentration across countries. It helps show whether countries with a greater plastic burden relative to cleanup effort also have worse air quality.

```{r}
pm25_vs_plastic <- pollution_data |>
  filter(!parent_company %in% c("Grand Total", "Unbranded", "null", "NULL")) |>
  group_by(country) |>
  summarize(
    total_plastic = sum(grand_total, na.rm = TRUE),
    total_volunteers = sum(volunteers, na.rm = TRUE),
    avg_pm25 = mean(`PM2.5_concentration`, na.rm = TRUE)
  ) |>
  mutate(
    # calculate plastic waste per volunteer
    plastic_per_volunteer = total_plastic / total_volunteers
  ) |>
  filter(total_volunteers > 0) |>
  filter(!is.na(avg_pm25)) |>
  filter(!is.na(plastic_per_volunteer))
```

Plot related to this:

```{r}
ggplot(pm25_vs_plastic, aes(x = plastic_per_volunteer, y = avg_pm25)) +
  geom_point(color = "lightgreen", size = 2) +
  geom_smooth(method = "lm", se = FALSE, color = "blue") +
labs(
  title = "Plastic Waste per Volunteer vs PM2.5 Concentration",
    x = "Plastic Waste per Volunteer",
    y = "Average PM2.5 Concentration"
  ) +
  theme_minimal()
```
Notes:
This graph shows the relationship between plastic waste per volunteer and the average PM2.5 concentration across countries. In the code I grouped the data by country, calculated total plastic waste, total volunteers, and average PM2.5, then created a new variable for plastic waste per volunteer. In the graph each point represents one country and the line of best fit shows the overall trend between the two variables. The points are pretty spread out so there does not appear to be a very strong relationship between plastic waste per volunteer and PM2.5 concentration. But the slight upward slope in the line shows that countries with more plastic waste per volunteer may have somewhat higher PM2.5 levels.
